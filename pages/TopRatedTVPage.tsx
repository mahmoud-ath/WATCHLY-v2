...
import { fetchTopRatedTVWithPage, convertTMDBTVToMovie } from '../services/tmdbService';
...
const TopRatedTVPage: React.FC = () => {
  ...
  useEffect(() => {
    loadTopRatedTV(currentPage);
  }, [currentPage]);

  const loadTopRatedTV = async (page: number) => {
    setStatus(LoadingState.LOADING);
    window.scrollTo({ top: 0, behavior: 'smooth' });
    
    try {
      const { results, total_pages } = await fetchTopRatedTVWithPage(page);
      const convertedShows = results.map(s => convertTMDBTVToMovie(s));
  ...
  if (status === LoadingState.ERROR) {
    return (
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-10">
        <ErrorMessage message="Failed to load top rated TV shows. Please try again." />
      </main>
    );
  }
  ...
  <span>Top Rated TV Series</span>
          ...
          <p className="text-slate-400 mt-2">
            Must-watch series · Page {currentPage} of {totalPages}
          </p>
        </div>
      </div>
