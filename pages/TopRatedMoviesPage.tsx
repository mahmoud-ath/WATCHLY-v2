...
import { fetchTopRatedMoviesWithPage, convertTMDBMovieToMovie } from '../services/tmdbService';
...
const TopRatedMoviesPage: React.FC = () => {
  ...
  const loadTopRatedMovies = async (page: number) => {
    setStatus(LoadingState.LOADING);
    window.scrollTo({ top: 0, behavior: 'smooth' });
    
    try {
      const { results, total_pages } = await fetchTopRatedMoviesWithPage(page);
  ...
  if (status === LoadingState.ERROR) {
    return (
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-10">
        <ErrorMessage message="Failed to load top rated movies. Please try again." />
      </main>
    );
  }
  ...
  Top Rated Movies
          ...
          <p className="text-slate-400 mt-2">
            Critically acclaimed films · Page {currentPage} of {totalPages}
          </p>
        </div>
      </div>
